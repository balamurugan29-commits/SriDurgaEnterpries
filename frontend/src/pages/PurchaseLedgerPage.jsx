...
export const PurchaseLedgerPage = () => {
  ...
  const openingBalance = useMemo(() => {
    const dealerKey = (filterDealer || '').trim().toUpperCase();
    let baseOpening = 0;
    
    // 1. Saved explicit opening balance for this store
    if (dealerKey) {
      for (const [k, val] of Object.entries(dealerOpenings)) {
        if (k.toUpperCase().includes(dealerKey) || dealerKey.includes(k.toUpperCase())) {
          baseOpening = Number(val) || 0;
          break;
        }
      }
    } else if (dealerOpenings['DEFAULT']) {
      baseOpening = Number(dealerOpenings['DEFAULT']) || 0;
    }

    // 2. Sum unpaid balances from prior purchases (dated before fromDate or previous FY 25-26)
    let priorUnpaid = 0;
    purchaseEntries.forEach(item => {
      const dealer = (item.dealerStoreName || item.supplierRemarks || '').toUpperCase();
      const matchesDealer = !dealerKey || dealer.includes(dealerKey);
      if (matchesDealer) {
        let isPrior = false;
        ...
        if (isPrior) {
          const total = Number(item.totalAmount) || 0;
          const paid = Number(item.paidAmount || item.passedAmount) || 0;
          priorUnpaid += Math.max(0, total - paid);
        }
      }
    });

    return baseOpening + priorUnpaid;
  }, [dealerOpenings, filterDealer, fromDate, purchaseEntries]);

  // Handler to set/save Opening Balance for current dealer
  const handleSaveOpeningBalance = (newAmount) => {
    const dealerKey = (filterDealer || 'DEFAULT').trim();
    const updated = {
      ...dealerOpenings,
              ...
              >
                ...
                <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
                  <h1 style={{ color: '#000000', fontSize: '1.45rem', fontWeight: 900, letterSpacing: '0.05em', margin: '0 0 0.35rem 0', textTransform: 'uppercase' }}>
                    {resolvedDealerName.toUpperCase()}
                  </h1>
                  <div style={{ color: '#000000', fontSize: '1.05rem', fontWeight: 800, marginBottom: '0.5rem' }}>
                    Supplier / Dealer Ledger for {resolvedDealerName.toUpperCase()}
                  </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem', fontFamily: 'Calibri, "Segoe UI", Arial, sans-serif' }}>
                  <thead>
                    <tr style={{ borderBottom: '1.5px solid #000000', color: '#000000', fontWeight: 800 }}>
                      ...
                      <th style={{ textAlign: 'left', padding: '6px 4px', width: '24%' }}>Name of Dealer/Store</th>
                  ...
                  <tbody>
                          ...
                          {hasPayment && (
                            <tr style={{ borderBottom: '1px solid #f3f4f6' }}>
                              ...
                              <td style={{ textAlign: 'center', padding: '5px 4px', color: '#4b5563' }}>
                                {formatStDate(l.paymentDate || l.invoiceDate)}
                              </td>
                    ...
                    <tr style={{ fontWeight: 900, fontSize: '0.95rem' }}>
                      ...
                      <td style={{ textAlign: 'right', padding: '8px 4px', borderBottom: '3px double #000000', color: '#16a34a' }}>
                        {Math.max(0, (openingBalance + totals.totalAmount) - totals.paidAmount).toFixed(2)}
                      </td>
