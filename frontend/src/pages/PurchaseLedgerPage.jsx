              ...
              <div style={{ background: 'rgba(192, 132, 252, 0.08)', padding: '0.875rem 1rem', borderRadius: '10px', border: '1px solid rgba(192, 132, 252, 0.3)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.72rem', color: '#94a3b8' }}>
                    Choose statement view & which party dues to include
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
                  ...
                  >
                    📊 Consolidated Summary Table (1 Row / Dealer)
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    📑 1 Dealer Per Page (Individual Statements)
                  </button>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    title="Include only parties with Pending Dues or Extra/Advance (Exclude ₹0.00)"
                  >
                    ⚡ Active Parties Only (Skip ₹0 Cleared)
                  </button>
                  <button
                    type="button"
                    ...
                    title="Include only parties where Balance Due > 0"
                  >
                    ⚠️ Pending Dues Only
                  </button>
                  <button
                    ...
                    title="Include only parties where Extra/Advance Paid < 0"
                  >
                    💳 Extra / Advance Only
                  </button>
                  <button
                    ...
                    style={{ padding: '0.3rem 0.75rem', fontSize: '0.75rem', borderRadius: '6px', fontWeight: 700 }}
                    title="Include All Parties (including ₹0.00 cleared)"
                  >
                    🌐 All Parties
                  </button>
                </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
                ...
                <button
                  ...
                  title="Show only parties with Pending Dues or Extra/Advance (Exclude ₹0.00)"
                >
                  ⚡ Active (Skip ₹0)
                </button>
                <button
                  ...
                  title="Show only parties with Pending Dues (> 0)"
                >
                  ⚠️ Pending Dues Only
                </button>
                <button
                  ...
                  title="Show only parties with Extra / Advance Paid (< 0)"
                >
                  💳 Extra / Advance Only
                </button>
                <button
                  ...
                  className={`btn ${statementBalanceFilter === 'ALL' ? 'btn-primary' : 'btn-outline'}`}
                  style={{ padding: '0.3rem 0.7rem', fontSize: '0.75rem', borderRadius: '6px', fontWeight: 700 }}
                  title="Show all parties including ₹0.00"
                >
                  🌐 All Parties
                </button>
              </div>
            ...
            <div style={{ padding: '1.25rem', overflowY: 'auto', background: '#334155' }}>
              ...
              >
                {/* Dynamic Statement Table: Consolidated Summary for ALL Suppliers, Individual 1-Page Statements, or Detailed FIFO */}
                {(() => {
                  ...
                  // =========================================================================
                  // MODE A & B: ALL SUPPLIERS (CONSOLIDATED OR INDIVIDUAL 1 PARTY PER PAGE)
                  // =========================================================================
                  if (isAllSuppliersMode) {
                    ...
                    const allConsolidatedSuppliers = Object.values(partyMap).map(d => {
                      ...
                      let running = opBal;
                      const entriesWithBal = sortedDealerEntries.map(e => {
                        const t = Number(e.totalAmount) || 0;
                        const p = Number(e.paidAmount || e.passedAmount) || 0;
                        running = (running + t) - p;
                    ...
                    const consolidatedSuppliers = allConsolidatedSuppliers.filter(sup => {
                      if (statementBalanceFilter === 'DUES_ONLY') return sup.netBalance > 0.009;
                      if (statementBalanceFilter === 'ADVANCE_ONLY') return sup.netBalance < -0.009;
                      if (statementBalanceFilter === 'ALL_NON_ZERO') return Math.abs(sup.netBalance) > 0.009;
                      return true; // 'ALL'
                    });
                      ...
                      return (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                          {consolidatedSuppliers.map((sup, pIdx) => (
                            ...
                            >
                              {/* 1. Page Header */}
                              <div style={{ textAlign: 'center', marginBottom: '1.5rem', borderBottom: '2px solid #000000', paddingBottom: '0.75rem' }}>
                                <h1 style={{ color: '#000000', fontSize: '1.55rem', fontWeight: 900, letterSpacing: '0.05em', margin: '0 0 0.25rem 0', textTransform: 'uppercase' }}>
                                ...
                                <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#000000', marginTop: '0.45rem', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
                                  SUPPLIER / DEALER STATEMENT OF ACCOUNT
                                </div>
                              </div>

                              {/* 2. ONE Unified Combined Centered Statement Card */}
                              <div style={{ border: '2px solid #000000', borderRadius: '8px', overflow: 'hidden', margin: '1.5rem 0', background: '#ffffff', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
                                
                                {/* Top Header Row: Party Name & Period */}
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', padding: '1rem 1.25rem', background: '#f8fafc', borderBottom: '1.5px solid #000000' }}>
                                  <div>
                                    <div style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Name of Dealer / Supplier:</div>
                                    <div style={{ fontSize: '1.35rem', fontWeight: 900, color: '#000000', marginTop: '2px' }}>{sup.dealerName}</div>
                                  </div>
                                ...
                                {/* Middle Rows: Summary Numbers Table */}
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.95rem', fontFamily: 'Calibri, "Segoe UI", Arial, sans-serif' }}>
                                  <tbody>
                                    {sup.openingBalance !== 0 && (
                                      <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                                        <td style={{ padding: '12px 18px', fontWeight: 700, color: '#334155' }}>Opening Balance</td>
                                        <td style={{ padding: '12px 18px', textAlign: 'right', fontWeight: 800, color: '#16a34a', fontSize: '1.05rem' }}>
                                          ₹{sup.openingBalance.toFixed(2)}
                                        </td>
                                      </tr>
                                    )}
                                    <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                                      <td style={{ padding: '12px 18px', fontWeight: 700, color: '#334155' }}>Total Purchases / Bills</td>
                                      <td style={{ padding: '12px 18px', textAlign: 'right', fontWeight: 800, color: '#0f172a', fontSize: '1.05rem' }}>
                                        ₹{sup.totalAmount.toFixed(2)}
                                      </td>
                                    </tr>
                                    <tr style={{ borderBottom: '2px solid #000000' }}>
                                      <td style={{ padding: '12px 18px', fontWeight: 700, color: '#334155' }}>Total Paid Amount</td>
                                      <td style={{ padding: '12px 18px', textAlign: 'right', fontWeight: 800, color: '#1e40af', fontSize: '1.05rem' }}>
                                ...
                                {/* Bottom Highlight Row: Closing Status & Amount */}
                                <div style={{ 
                                  display: 'flex', 
                                  alignItems: 'center', 
                                  justifyContent: 'space-between', 
                                  ...
                                  <div>
                                    ...
                                    <div style={{ fontSize: '1.05rem', fontWeight: 900, color: sup.netBalance > 0 ? '#991b1b' : (sup.netBalance < 0 ? '#1e40af' : '#166534'), marginTop: '3px' }}>
                                      {sup.netBalance > 0 
                                  ...
                                  <div style={{ textAlign: 'right' }}>
                                    <div style={{ fontSize: '1.75rem', fontWeight: 900, color: sup.netBalance > 0 ? '#dc2626' : (sup.netBalance < 0 ? '#1e40af' : '#16a34a') }}>
                                      ₹{Math.abs(sup.netBalance).toFixed(2)}
                                    </div>
                                    {sup.netBalance < 0 && (
                                      <span style={{ fontSize: '0.75rem', fontWeight: 800, background: '#dbeafe', color: '#1e40af', padding: '2px 8px', borderRadius: '4px' }}>
                                        EXTRA / ADVANCE
                                      </span>
                                    )}
                                  </div>
                              ...
                              {/* 3. Formal Authorisation Signatures */}
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '3rem', paddingTop: '1.25rem', borderTop: '1px solid #e2e8f0', fontSize: '0.875rem', fontWeight: 800 }}>
                    ...
                    // ---------------------------------------------------------------------
                    // FORMAT 2: CONSOLIDATED MASTER SUMMARY TABLE (1 ROW PER DEALER)
                    // ---------------------------------------------------------------------
                    return (
                      <div>
                        {/* Statement Header */}
                        <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
                          ...
                          <div style={{ color: '#000000', fontSize: '1.05rem', fontWeight: 800, marginBottom: '0.5rem' }}>
                            Supplier / Dealer Ledger for: <span style={{ textDecoration: 'underline' }}>ALL SUPPLIERS & DEALERS</span>
                          </div>
